...
def create_one():
    ...
    if not all(key in data for key in ['id_fiscal', 'razon_social', 'nombre', 'apellido', 'direccion', 'telefono', 'mail', 'categoria_fiscal']):
    ...
    if insert_result and insert_result.inserted_id:
        return redirect(url_for('clientes.index'))  # Redirect to the clients index page
    else:
